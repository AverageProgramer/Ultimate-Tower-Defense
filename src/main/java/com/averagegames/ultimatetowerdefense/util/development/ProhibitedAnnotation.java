...
public final class ProhibitedAnnotation {
    ...
    public static void verify(@NotNull final Class<?> caller, @NotNull final Method annotated) {
        ...
        assert annotated.isAnnotationPresent(Prohibited.class) : STR."Method \{annotated} in class \{annotated.getDeclaringClass()} is not annotated with \{Prohibited.class}";
        ...
        if (!allowed) {
            ...
            throw new ProhibitedAccessException(STR."Class [\{caller}] is prohibited from accessing method [\{annotated}]");
        }
    }
}