...
public final class ProhibitedAnnotation {
    ...
    public static void verify(@NotNull final Class<?> caller, @NotNull final Method annotated) {
        ...
        // The reason this is being used here is that this expression will always evaluate to true.
        ...
        for (var prohibited : annotated.getAnnotation(Prohibited.class).value()) {
            ...
            for (var subclass : new Reflections(new ConfigurationBuilder().addScanners(Scanners.SubTypes).addUrls(ClasspathHelper.forClass(prohibited))).getSubTypesOf(prohibited)) {
