...
public final class SpecificAnnotation {
    ...
    public static void verify(@NotNull final Class<?> caller, @NotNull final Method annotated) {
        ...
        // The reason assertions are being used here is that this expression should always evaluate to true.
        assert annotated.isAnnotationPresent(Specific.class) : STR."Method \{annotated} in class \{annotated.getDeclaringClass()} is not annotated with \{Specific.class}";
        ...
        for (Class<?> specified : annotated.getAnnotation(Specific.class).value()) {
            ...
            for (Class<?> subclass : new Reflections(new ConfigurationBuilder().addScanners(Scanners.SubTypes).addUrls(ClasspathHelper.forClass(specified))).getSubTypesOf(specified)) {
        ...
        if (!allowed) {
            ...
            throw new UnspecifiedAccessException(STR."Class [\{caller}] does not have specified access to method [\{annotated}]");
        }
    }
}